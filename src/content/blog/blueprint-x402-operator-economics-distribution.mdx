...
A blueprint is a packaged service definition with a typed job interface. An operator is the person or service responsible for running a job. x402 is a payment protocol for machine-to-machine requests. When a client pays an x402 Blueprint job, the USDC moves to the operator's `pay_to` address on the settlement chain. That's the end of the client's story. It's the beginning of the operator's.
...
| Step | Contract or module | Decision being made | Why it matters |
|---|---|---|---|
| x402 settlement | Operator gateway and settlement chain | Did the client pay the quoted amount before the job ran? | Prevents the operator from burning compute on unpaid calls |
| Fee intake | `ServiceFeeDistributor` | Is this native ETH or an ERC-20, and did the Tangle contract call it? | Keeps payment accounting tied to the protocol entry point |
| Timing | `StreamingPaymentManager` | Should the payment distribute now or drip over the service TTL? | Prevents late delegators from claiming revenue earned before they joined |
| Pool split | All-mode and Fixed-mode score totals | How much goes to broad operator backing vs blueprint-specific backing? | Rewards the kind of delegated risk that supported the service |
| Claim math | Accumulator-per-score accounting | What can each delegator claim without looping through every account? | Makes fee distribution scalable as the delegator set grows |
| Fallback | Treasury route | Where does the fee go if no score exists? | Avoids stranded funds and makes misconfigured operators visible |
...
The exposure module shown above was inlined into the payments calculator. The live equivalent lives in [`tnt-core/src/core/PaymentsEffectiveExposure.sol`](https://github.com/tangle-network/tnt-core/blob/main/src/core/PaymentsEffectiveExposure.sol).
...
For the ingress side of this path, read [Blueprint SDK x402 payments](/blog/blueprint-sdk-x402-payments-runnable-jobs). For quote-bound operator commitments, read [RFQ job quotes on Tangle](/blog/rfq-job-quotes-tangle-operator-accountability).
...
### Does x402 decide how operators and delegators split revenue?

No. x402 proves and settles the client payment. The Tangle protocol distributes the service fee afterward through `ServiceFeeDistributor`, streaming logic, USD-weighted scores, and blueprint selection mode. Treat x402 as payment intake, not the full economics engine.

### Why does Tangle weight rewards by USD exposure instead of token count?

Raw token counts are not comparable across assets. USD-weighted exposure lets ETH, stablecoin, TNT, and other accepted assets compete through a common value measure, then adjusts reward share by how much of that capital is in practice committed to the service.

### Can a delegator bond after a job starts and capture the old fee?

Not for time-bounded services. Streaming payments drip against the score ratios in effect at drip time. A delegator who joins later starts earning on future drips, not on revenue already earned by the previous backing set.

### When should an operator care about All mode versus Fixed mode?

All mode is better when delegators trust the operator's whole service roster. Fixed mode is better when delegators want exposure to a specific blueprint. Operators should explain which pool backs which service because that choice affects fee share, risk, and slashing exposure.
