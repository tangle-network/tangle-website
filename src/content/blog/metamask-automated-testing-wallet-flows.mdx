...
MetaMask's public docs describe the connection model in [MetaMask Connect](https://docs.metamask.io/metamask-connect/). Ethereum provider behavior is specified by [EIP-1193](https://eips.ethereum.org/EIPS/eip-1193). Structured signing is specified by [EIP-712](https://eips.ethereum.org/EIPS/eip-712). Your automated test should save evidence at the user prompt, alongside the response the dapp receives.

## Run Shape

Drive the wallet flow through the browser test runner and save the prompt, rejection, retry, and final state. A reader can adapt that sequence to their own environment without a local setup transcript.
...
| --- | --- |
| extension | installed and enableed before run |
