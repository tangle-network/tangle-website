...
| Phase | Facilitator still does | Gateway verifies directly | Trust assumption removed |
|---|---|---|---|
| 1. Receipt verification | Settles payment and returns success | Transaction receipt exists and succeeded on-chain | Facilitator cannot fabricate settlement |
| 2. Local verification | Submits the settlement transaction | Signature, balance, authorization window, and receipt | Facilitator cannot censor by lying about validity |
| 3. Self-settlement | Nothing in the hot path | Gateway submits and confirms `transferWithAuthorization` | Operator availability no longer depends on a third-party HTTP service |
| 4. Blueprint facilitation | Becomes a staked service, not a singleton | Tangle result reporting and slashing evidence | Incorrect facilitation becomes economically punishable |
...
For the current gateway integration surface, read [Blueprint SDK x402 payments](/blog/blueprint-sdk-x402-payments-runnable-jobs). For production rollout risks around payment configuration, use the [x402 deployment checklist](/blog/x402-blueprint-production-deployment-checklist).
