...
x402 is a payment protocol for machine-to-machine requests. *Day 2 of the Tangle Re-Introduction Series*
...
This is where Tangle's blueprints come in. They provide the execution layer that x402 payments pay for, with built-in [verification](/blog/how-tangle-verifies-work) and economic consequences for misbehavior.
...
**For TEE execution:** Check the attestation. Hardware manufacturers sign attestations proving code ran inside an enclave. See [Trusted Execution on Tangle](/blog/trusted-execution-on-tangle) for how the SDK integrates TEE across cloud providers.
...
**x402 handles the payment.** An agent discovers a service, negotiates price, and pays in stablecoins over HTTP. The protocol is lightweight: an HTTP 402 response contains payment requirements, the agent pays, the service enables. No API keys, no accounts, no billing systems.
...
**The combination is useful.** An agent can autonomously find compute providers, pay for services, and have cryptographic guarantees that it got what it paid for. All at machine speed, with no human intervention.
...
We're building blueprint templates that integrate [x402 payment verification](/blog/blueprint-sdk-x402-payments-runnable-jobs) directly into the service lifecycle. An x402 payment triggers service creation; job completion triggers payment release; failed verification triggers refund. The economic loop is closed and automatic.
...
A blueprint is a reusable template that defines a type of service: its computation logic, operator registration requirements, verification hooks, and slashing conditions. A single blueprint can power thousands of service instances with different operators and configurations.

### How do blueprint developers earn revenue?
Developers receive a portion of every transaction processed through their blueprint. When customers pay for jobs, the smart contract distributes fees automatically to operators, blueprint developers, delegators, and the protocol treasury.

### Can AI agents use blueprints autonomously?
Yes. Agents can discover blueprints via the on-chain registry, evaluate operator stakes and reputation, create service instances, submit jobs, and verify results, all without human intervention. Combined with x402 payments, the entire flow from discovery to settlement is automated.

### What verification mechanisms do blueprints support?
Blueprints support multiple approaches: deterministic output comparison across operators, cryptographic proofs (ZK), TEE attestation, consistency checking, and canary prompts. The developer chooses which mechanisms match their service's trust requirements.

### What happens if an operator fails to deliver?
The smart contract executes consequences immediately. Verification failure triggers slashing in the same block. There are no lawyers, no arbitration, and no delay. Funds are held in programmable escrow with explicit release conditions.
