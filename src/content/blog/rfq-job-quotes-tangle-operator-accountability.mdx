...
description: "How Tangle RFQ job quotes become signed commitments with quoted-operator result gates, exact-price payment, and a 3.5-day dispute window."
date: '2026-03-28'
author: Drew Stone
tags:
  - x402
...
An operator is the person or service responsible for running a job. Imagine hiring a contractor. They give you a written estimate: specific job, specific price, signed. You hand them a deposit. They don't show up. What do you do?
...
What makes this non-obvious: the accountability mechanism also concerns who can submit the result. It's about *who can submit a result*. Only operators who signed a quote for a specific job can submit results for that job. Any other operator calling `submitResult` gets a hard revert. This single design decision closes the free-rider problem and makes the quote a genuine commitment, not a suggestion.
...
| Lifecycle point | Evidence stored | Failure it prevents |
|---|---|---|
| Quote creation | EIP-712 signature over `serviceId`, `jobIndex`, `price`, `timestamp`, and `expiry` | Operator later claiming they never priced the job |
| Job submission | `_jobQuotedOperators` and `_jobQuotedPrices` keyed by `serviceId` and `callId` | Unquoted operators racing in after seeing a profitable job |
| Result submission | `NotQuotedOperator` revert for addresses outside the quote set | Payment going to someone who never accepted the price |
| Finalization | RFQ-specific distribution at each quoted price | Pool splitting that ignores the original operator commitment |
| Non-delivery | Slash proposal with evidence and dispute window | A quoted operator taking the job and disappearing without recourse |
...
The source reports `MAX_QUOTE_AGE = reported 1 hour` from [`ProtocolConfig.sol`](https://github.com/tangle-network/tnt-core/blob/main/src/config/ProtocolConfig.sol), though deployments can override this. The effect: even if an operator sets a generous `expiry`, the protocol also enforces that the quote was freshly generated. A reported example with a 24-hour `expiry` and a `timestamp` from reported 90 minutes ago fails at step 2.
...
Unlike keeper networks, where any registered keeper can fill a task and the question is who gets there first, the RFQ model pre-commits identity. The operator who priced the job is the only one who can deliver it. That's what makes the price commitment meaningful.
...
`slashBps` is capped by `maxSlashBps` in the protocol config. Setting it above `BPS_DENOMINATOR` (10000) reverts. The `evidence` field is a `bytes32` reference, often an IPFS CID , the contract stores it but does not validate the format.

The source describes a dispute window of `DISPUTE_WINDOW_ROUNDS = 14` rounds at `ROUND_DURATION_SECONDS = 21,600` seconds per round: exactly reported 3.5 days. During this window, the operator or a `SLASH_ADMIN_ROLE` address can call `disputeSlash(slashId, reason)` to contest the proposal. After the window closes, `executeSlash` reduces the operator's stake:
...
The practical difference: x402 is well-suited for stateless pay-per-call endpoints where any backend can serve the request and failure means a new request. RFQ is suited for jobs where operator identity matters, where you want price certainty before commit, and where you need protocol-level accountability for failure.
...
For stateless HTTP payment, read [Blueprint SDK x402 payments](/blog/blueprint-sdk-x402-payments-runnable-jobs). For what happens after fees enter the protocol, read [operator economics after payment](/blog/blueprint-x402-operator-economics-distribution).

## Decision

Choose RFQ when the caller needs one named operator to commit to a quoted job and face a protocol consequence for non-delivery. Choose x402 when any healthy backend can serve a stateless request and a failed attempt can be retried. Both can share one product, but they answer different trust questions.
...
In this reported example, it fails the freshness check. `MAX_QUOTE_AGE = 1 hour` means the protocol enforces that `block.timestamp <= quote.details.timestamp + maxQuoteAge`. A quote with a 24-hour `expiry` but a `timestamp` from reported 90 minutes ago is rejected as too old, regardless of the `expiry` field.
