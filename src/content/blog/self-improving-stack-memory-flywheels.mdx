...
> **Short answer:** Agent memory becomes useful when a past run produces a scoped note, the next run retrieves it in the right situation, and a comparison shows that the result improved.

**Memory** is information carried from one agent run to another.
A **trace** is the record of a past run.
**Retrieval** is the step that chooses which saved information to show the agent now.
Saving more text is not the same as learning; an incorrect note can repeat the same failure.

Remembering more is not learning.

Learning means the next run changes in the right direction.

Memory is one way to change the next run without changing model weights. It lets an agent carry evidence, preferences, decisions, failures, and procedures across episodes. That makes memory useful. It also makes memory dangerous.
...
Then the evaluator measures whether the retrieval in practice helped.
...
Without that ablation, memory success is often retrieval theater.
...
This is often the difference between an agent that keeps repeating a class of mistake and one that in practice compounds.
...
A memory can say that a repository should be inspected before it is edited.

A skill can make that lesson executable by naming its inputs, preconditions, inspection steps, and checks for an unfinished merge or rebase.
...
Knowledge poisoning is more serious than a missing fact in the agent's context.
...
It may be a larger prompt with a longer memory leak.
