...
A decentralized compute protocol defines how work is requested, priced, assigned to operators, executed, verified, and returned with evidence.

### What is a Tangle Blueprint?

A Tangle Blueprint is a service definition that operators can register for and run, including job interfaces, runtime requirements, and verification expectations.

### Why does compute need protocol rules?

Protocol rules make pricing, operator accountability, verification, and failure behavior explicit instead of hiding them behind one provider's API.

### What workloads fit Blueprints?

Inference, sandbox execution, browser automation, code audits, data jobs, TEE workloads, and bounded agent services can fit the Blueprint model.
