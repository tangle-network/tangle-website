...
The Blueprint protocol is Tangle's model for defining services that independent operators can run under protocol rules.

### Is a Blueprint the same as an API?

No. A normal API is usually run by one provider. A Blueprint is a service definition that operators can register for, run, price, and verify.

### Who runs a Blueprint?

Independent operators run Blueprints after registering, configuring runtime settings, and meeting the service's operating requirements.

### What makes a Blueprint useful?

A useful Blueprint has a clear job interface, runtime requirements, pricing model, verification path, operator instructions, and failure behavior.
