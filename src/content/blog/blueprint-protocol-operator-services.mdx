...
An operator is the person or service responsible for running a job. Blueprint protocol is the on-chain and operator-facing side of Tangle. It should not be confused with Blueprint Agent, the developer onboarding product at `ai.tangle.tools`. A protocol Blueprint is a reusable service definition. Operators register for it, run the service, accept jobs, and receive payment according to the network rules.
