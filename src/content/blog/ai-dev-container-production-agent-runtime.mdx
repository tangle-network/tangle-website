...
An AI dev container is an isolated development workspace where an agent can read files, run commands, edit code, and leave a reviewable execution trace.
...
It provides managed environment creation, command execution, streaming session state, snapshots, cleanup, and trace export for agent workflows.

### When should I use a managed sandbox?

Use one when agent output may affect production code, customer data, deployments, funds, credentials, or any system where a local unchecked run is too risky.
