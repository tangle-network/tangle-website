...
summary: >-
  ...
  CVM for hardware-isolated execution with attestation and sealed secrets.
date: '2026-03-02'
author: Drew Stone
tags:
  - tee
...
Previous posts covered [why decentralized AI infrastructure matters](/blog/why-ai-infrastructure-needs-decentralization), [how blueprints work](/blog/how-blueprints-work), [verification mechanisms](/blog/how-tangle-verifies-work), [building from idea to production](/blog/building-on-tangle-from-idea-to-production), and [AI services with inference and sandboxes](/blog/building-ai-services-on-tangle). This post covers what just shipped: first-class TEE support in the Blueprint SDK.
...
Primary provider references: [AWS Nitro Enclaves](https://docs.aws.amazon.com/enclaves/latest/user/nitro-enclave.html), [Google Cloud Confidential Space](https://cloud.google.com/confidential-computing/confidential-space/docs), and [Azure Confidential VMs](https://learn.microsoft.com/en-us/azure/confidential-computing/confidential-vm-overview).
...
If you're building a blueprint that handles sensitive data, model weights, or private inference — whether following the [idea-to-production path](/blog/building-on-tangle-from-idea-to-production) or integrating [x402 payments](/blog/blueprint-sdk-x402-payments-runnable-jobs) — TEE is how you prove to users that their data stays confidential. The SDK handles the complexity of multi-cloud provisioning, attestation verification, and sealed secret management so your blueprint code stays focused on the actual service logic.

## FAQ

### Which cloud providers does the Blueprint SDK support for TEE?
AWS Nitro Enclaves, GCP Confidential Space (AMD SEV-SNP and Intel TDX), and Azure Confidential VMs (DCasv5/ECasv5 series). Each is feature-gated so the default build stays lightweight. Direct hardware mode also supports bare-metal Intel TDX and AMD SEV-SNP.

### Why can't I recreate a TEE container when config changes?
Recreating the container invalidates the attestation, breaks sealed secrets, and loses the on-chain deployment ID. Instead, secrets flow through a key exchange protocol: the TEE generates an X25519 key pair embedded in the attestation report, and clients encrypt secrets to that key.

### What is Direct mode vs Remote mode?
Direct mode runs the blueprint runner itself inside a TEE with device passthrough, giving the highest integrity with fewest network trust links. Remote mode has the runner provision workloads into cloud TEE instances via provider APIs, managing the lifecycle externally.

### How does attestation verification work across providers?
Each provider has different evidence formats, but the `AttestationVerifier` trait normalizes them. Built-in verifiers check provider type, debug mode flags, measurement hashes, and attestation freshness. The on-chain contract stores `keccak256(attestationJsonBytes)` so anyone can verify independently.

---

[Blueprint SDK on GitHub](https://github.com/tangle-network/blueprint) · [TEE crate source](https://github.com/tangle-network/blueprint/tree/main/crates/tee) · [Previous: Building AI Services on Tangle](/blog/building-ai-services-on-tangle)
