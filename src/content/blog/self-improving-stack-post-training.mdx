---
title: 'Post-Training Agents: When the Model Itself Is Mutable'
...
> **Short answer:** Post-training changes the model’s learned behavior rather than only changing the prompt around it.

**Model weights** are the learned parameters that produce the model’s behavior.
**Post-training** updates those parameters or an adapter attached to them.
That can teach a broad behavior, but it also makes rollback, privacy review, testing, and ownership more demanding than changing a prompt or skill.
In this article, a **harness** is the surrounding execution software, and a **trace** is the record of one run.
...
This is useful when demonstrations are clean and the task distribution is stable.
...
They are comparatively easy to inspect because a reviewer can compare the changed files, replay a recorded run, inspect the score change, and return to the previous version.
...
Without that lineage, model-level self-improvement is not a controlled system. It is drift with a training budget.
