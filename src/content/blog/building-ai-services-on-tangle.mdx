...
The previous post covered [the general developer experience](/blog/building-on-tangle-from-idea-to-production). This one gets specific: how to build the two AI services that matter most right now.
...
Tangle addresses these through [verification mechanisms](/blog/how-tangle-verifies-work) and economic accountability. Let's see how that works in practice.

| Service type | What the customer needs proved | Tangle mechanism | Main limitation |
|---|---|---|---|
| Inference | The claimed model and policy ran, and the operator did not silently substitute cheaper compute | TEE attestation for model loading, canary checks, optional multi-operator quorum | GPU confidentiality and model-quality verification are still workload-specific |
| Sandbox execution | Untrusted code ran inside the requested policy boundary and produced an auditable result | Isolated runtime, resource limits, execution trace, operator stake | Network and filesystem policy must be explicit or the sandbox becomes a general-purpose host |
...
**For TEE-enabled execution:** Hardware attestation proves the sandbox ran the code correctly. For a deep dive on TEE integration, see [Trusted Execution on Tangle](/blog/trusted-execution-on-tangle). This is the strongest guarantee but requires TEE-capable operators.
...
### Can I run large language models inside a TEE on Tangle?
Yes, but with constraints. Intel SGX has ~256MB EPC, so large models require AMD SEV-SNP instances or model sharding. GPU TEE support (NVIDIA H100 Confidential Computing) exists but isn't widely deployed yet. For now, multi-operator verification provides an alternative for GPU inference.

### How does Tangle prevent an operator from substituting a cheaper model?
Three mechanisms: TEE attestation verifies the model hash at load time, canary prompts with known expected outputs detect behavioral differences between models, and slashing conditions penalize operators whose attestations or canary checks fail.

### What languages can I run in the sandbox service?
The sandbox supports Python, JavaScript (V8 isolate), Rust, Go, and WebAssembly. Each language has a runtime optimized for isolated execution. WASM provides the strongest isolation guarantees due to its sandboxed memory model.

### How do inference and sandbox services chain together?
A request flows through inference (generate code) then sandbox (execute code). Both steps produce attestations and execution traces, creating a full accountability chain. The customer receives the result, the generated code, and cryptographic proof for each step.

### What happens if sandbox code tries to access the network or filesystem?
Network and filesystem access are controlled by policy. `NetworkPolicy::None` blocks all network access, `AllowList` permits only specified hosts, and `FilesystemPolicy` restricts disk operations. Each execution runs in a fresh container that is destroyed afterward.
