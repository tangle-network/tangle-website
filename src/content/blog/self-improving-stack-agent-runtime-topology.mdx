...
| Topology capability | If it is first-class | If it is only prose |
|---|---|---|
| Fanout | Runtime can launch independent workers and preserve separate traces | The model says "parallelize" but calls tools serially |
| Selection | A verifier, score, or policy chooses among candidates | The model narrates a preference with no enforceable winner |
| Checkpoint | Work can resume, replay, or branch from a known state | Long runs restart from context summaries and lose evidence |
| Human gate | Approval blocks high-risk actions before execution | The model merely promises to ask before it acts |
| Budget control | Tokens, tools, wall time, and dollars are bounded by the runtime | The prompt asks the model to be efficient |
...
Compare topology candidates against compute-matched baselines with full traces, cost ceilings, deterministic checks, and safety gates. The next post on [multi-agent coordination](/blog/self-improving-stack-multi-agent-coordination/) covers role contracts; [test-time compute](/blog/self-improving-stack-test-time-compute/) covers the budget baseline.
