...
This article walks through three environments: dev (local, loopback), staging (remote, testnet), and production (mainnet, TEE enforced). At each stage, a small set of fields changes. Knowing which fields those are, and why they change, is the difference between a smooth launch and an expensive config mistake.

If you need the gateway wiring first, start with [Blueprint SDK x402 payments](/blog/blueprint-sdk-x402-payments-runnable-jobs). If you are enabling confidential execution, pair this checklist with [x402 and TEE production gating](/blog/blueprint-tee-x402-production-gating).
...
| Readiness area | Caught at config load | Silent failure if wrong | Promotion proof |
|---|---|---|---|
| Payment destination | Address parses as EVM-shaped | Funds settle to the wrong wallet | Staging receipt pays the intended operator address |
| Network and token | CAIP-2 and address strings parse | Facilitator or token contract rejects settlement | Sepolia payment succeeds, then mainnet addresses are changed in one review |
| Pricing | TOML value parses | ETH/USDC drift makes jobs underpriced or overpriced | Deployment log records the rate source and review date |
| Job policy | Missing required fields reject | New job inherits the wrong default mode | Every job index has an explicit policy row |
| TEE gate | Some invalid combinations reject | `preferred` or local-only attestation allows downgrade | Provisioned attestation hash exists before `on_chain_verification = true` |
...
## Which config fields cost you money?
...
1. **Submit a test request** to the service and confirm you receive a 402 response with a valid quote. The quote TTL is `quote_ttl_secs` (reported default 300 seconds); complete the verification within that window.
