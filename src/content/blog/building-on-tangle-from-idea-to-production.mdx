...
The previous posts covered [why decentralized infrastructure matters](/blog/why-ai-infrastructure-needs-decentralization) and [how verification works](/blog/how-tangle-verifies-work). This one is practical: how do you actually build something?
...
When you build on Tangle, you're creating a **blueprint**: a template that defines a type of service. For a deeper look at blueprint architecture, see [How Blueprints Work](/blog/how-blueprints-work). Operators register to run your blueprint. Customers pay to use instances of your service. You earn a share of every transaction.
...
2. **Clone an example:** [github.com/tangle-network/blueprint](https://github.com/tangle-network/blueprint/tree/main/examples)
...
**Do I need to know Rust to build on Tangle?**
The Blueprint SDK is Rust-based, so Rust proficiency is required. If you're learning Rust and Tangle simultaneously, expect a steeper learning curve. The SDK handles protocol communication and job routing, so you focus on business logic rather than low-level blockchain integration.

**How do operators find and decide to run my blueprint?**
Operators discover blueprints via the on-chain registry. They evaluate profitability based on fee structure, hardware requirements, and complexity. Making operator setup easy and setting realistic fee structures increases the chance operators will register.

**What happens if my verification logic has a bug?**
Operators can cheat without getting slashed. Verification is the most critical code in your blueprint. Use multiple verification approaches (redundancy plus attestation), conservative slashing thresholds, and exhaustive testing before mainnet deployment.

**Can I run my own operators during the early phase?**
Yes, and it's recommended. Running your own operators initially solves the cold-start problem (zero operators means zero service). As external operators register, you can phase out your own.

**What's the difference between testnet and mainnet deployment?**
Testnet uses free test TNT tokens, lower stake requirements, and no SLAs. Mainnet uses real TNT, production-level stake minimums, and operator-defined SLAs. Always validate with real operators on testnet before launching on mainnet.
...
- [Example Blueprints](https://github.com/tangle-network/blueprint/tree/main/examples)
