...
Browser automation standards such as [WebDriver](https://www.w3.org/TR/webdriver2/), browser inspection surfaces such as the [Chrome DevTools Protocol](https://chromedevtools.github.io/devtools-protocol/), and frameworks such as [Playwright](https://playwright.dev/docs/intro) remain the base layer. Natural language should sit above them, not pretend the browser is magic.
...
Natural language test automation turns an English acceptance goal into a real browser run with actions, screenshots, observations, and a verdict.

### Can product managers write these tests?

Yes, if the goal includes a start point, important steps, final visible condition, and evidence requirement. Vague prompts still create vague tests.

### Does natural language testing work with CI?

Yes. The run should return pass, fail, blocked, or inconclusive with artifacts that CI and reviewers can inspect.

### How does Tangle Browser Agent fit?

Tangle Browser Agent provides the browser automation path, goal execution, artifact capture, and evidence loop behind natural language tests.
