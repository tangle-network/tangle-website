...
Wallet integrations often use the provider pattern described by [EIP-1193](https://eips.ethereum.org/EIPS/eip-1193). Typed signing commonly uses [EIP-712](https://eips.ethereum.org/EIPS/eip-712). Test evidence should show what the user was asked to approve alongside the app's post-click state.

## Agent Run Pattern

Run the wallet flow in an isolated test environment and save the app state, wallet prompt, and final transaction result. The test plan matters more than a shell recipe.
...
The public [MetaMask developer docs](https://docs.metamask.io/metamask-connect/) are a useful baseline for wallet connection behavior. Your test artifacts should show how your app in practice presents that behavior.
...
| wallet locked | app waits or prompts user to enable |
