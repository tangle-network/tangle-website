...
An LLM sandbox environment is an isolated runtime where an agent can execute tools, run commands, edit files, stream state, and preserve evidence.

### Why do LLM agents need sandboxes?

Tool-using agents can change files, call APIs, install packages, or leak data. A sandbox limits blast radius and records what happened.

### Does a sandbox prevent every security issue?

No. Security also depends on credential scope, network policy, filesystem boundaries, review gates, and cleanup behavior.

### What does Tangle Sandbox SDK provide?

It provides a managed environment and session layer for agent execution, command streaming, snapshots, cleanup, and trace evidence.
